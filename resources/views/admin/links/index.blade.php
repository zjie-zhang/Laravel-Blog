        ...
        <div class="result_wrap">
            <div class="result_content">
                <table class="list_tab">
                    ...
                    <tr>
                        <td class="tc">
                            <input type="text" name="ord[]" value="{{$v['link_order']}}" onchange="changeOrder(this,{{$v['id']}})">
                        </td>
                        <td class="tc">{{$v['id']}}</td>
                        <td>{{$v['link_name']}}</td>
                        <td>{{$v['link_title']}}</td>
...
<script>
    function changeOrder(obj,id) {
        var id = id;
        var order = $(obj).val();
        $.post( "{{url('admin/links/changeorder')}}", {'_token':"{{csrf_token()}}",'id':id,'link_order':order},function (data) {
            if(data.num == 0){
